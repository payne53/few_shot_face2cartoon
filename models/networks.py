...
class ResnetGenerator(nn.Module):
    def __init__(self, ngf=64, img_size=256, light=False):
        ...
        self.ConvBlock1_1 = nn.Sequential(nn.ReflectionPad2d(3),
                                        nn.Conv2d(3, ngf, kernel_size=7, stride=1, padding=0, bias=False),
                                        nn.InstanceNorm2d(ngf),
                                        nn.ReLU(True))

        self.HourGlass1_1 = HourGlass(ngf, ngf)

        self.ConvBlock1_2 = nn.Sequential(nn.ReflectionPad2d(3),
                                          nn.Conv2d(3, ngf, kernel_size=7, stride=1, padding=0, bias=False),
                                          nn.InstanceNorm2d(ngf),
                                          nn.ReLU(True))

        self.HourGlass1_2 = HourGlass(ngf, ngf)

        self.ConvBlock1_3 = nn.Sequential(nn.ReflectionPad2d(3),
                                          nn.Conv2d(3, ngf, kernel_size=7, stride=1, padding=0, bias=False),
                                          nn.InstanceNorm2d(ngf),
                                          nn.ReLU(True))

        self.HourGlass1_3 = HourGlass(ngf, ngf)
        ...
        self.gap_fc_new = nn.Linear(ngf * 4, 4)
        self.gmp_fc_new = nn.Linear(ngf * 4, 4)
        ...
        self.HourGlass4_1 = HourGlass(ngf, ngf, False)

        self.ConvBlock2_1 = nn.Sequential(nn.ReflectionPad2d(3),
                                        nn.Conv2d(3, 3, kernel_size=7, stride=1, padding=0, bias=False),
                                        nn.Tanh())

        self.HourGlass4_2 = HourGlass(ngf, ngf, False)

        self.ConvBlock2_2 = nn.Sequential(nn.ReflectionPad2d(3),
                                          nn.Conv2d(3, 3, kernel_size=7, stride=1, padding=0, bias=False),
                                          nn.Tanh())

        self.HourGlass4_3 = HourGlass(ngf, ngf, False)

        self.ConvBlock2_3 = nn.Sequential(nn.ReflectionPad2d(3),
                                          nn.Conv2d(3, 3, kernel_size=7, stride=1, padding=0, bias=False),
                                          nn.Tanh())
    ...
    def forward(self, x, label):
        if label == 0:
            ...
            x = self.HourGlass1_1(x)
        elif label == 2:
            x = self.ConvBlock1_2(x)
            x = self.HourGlass1_2(x)
        elif label == 3:
            x = self.ConvBlock1_3(x)
            x = self.HourGlass1_3(x)

        x = self.HourGlass2(x)

        x = self.DownBlock1(x)
        x = self.DownBlock2(x)  # 1, 128, 64, 64

        x = self.EncodeBlock1(x)    # 1, 128, 64, 64
        content_features1 = F.adaptive_avg_pool2d(x, 1).view(x.shape[0], -1)    # 1, 128
        x = self.EncodeBlock2(x)    # 1, 128, 64, 64
        content_features2 = F.adaptive_avg_pool2d(x, 1).view(x.shape[0], -1)    # 1, 128
        x = self.EncodeBlock3(x)    # 1, 128, 64, 64
        content_features3 = F.adaptive_avg_pool2d(x, 1).view(x.shape[0], -1)    # 1, 128
        x = self.EncodeBlock4(x)    # 1, 128, 64, 64
        content_features4 = F.adaptive_avg_pool2d(x, 1).view(x.shape[0], -1)    # 1, 128
        ...
        gap_logit = self.gap_fc_new(gap.view(x.shape[0], -1))
        gap_weight = list(self.gap_fc_new.parameters())[0][label]
        ...
        gmp_logit = self.gmp_fc_new(gmp.view(x.shape[0], -1))
        gmp_weight = list(self.gmp_fc_new.parameters())[0][label]

        gmp_weight = gmp_weight.unsqueeze(2).unsqueeze(3)
        gmp = x * gmp_weight

        cam_logit = torch.cat([gap_logit.unsqueeze(1), gmp_logit.unsqueeze(1)], 1)
        x = torch.cat([gap, gmp], 1)
        x = self.relu(self.conv1x1(x))  # 1, 128, 64, 64

        heatmap = torch.sum(x, dim=1, keepdim=True)

        if self.light:
            x_ = F.adaptive_avg_pool2d(x, 1)
            style_features = self.FC(x_.view(x_.shape[0], -1))    # 1, 128
        else:
        ...
        x = self.DecodeBlock4(x, content_features1, style_features) # 1, 128, 64, 64
        ...
        if label == 0:
            ...
            x = self.HourGlass4_1(x)
            out = self.ConvBlock2_1(x)
        elif label == 2:
            x = self.HourGlass4_2(x)
            out = self.ConvBlock2_2(x)
        elif label == 3:
            x = self.HourGlass4_3(x)
            out = self.ConvBlock2_3(x)

        return out, cam_logit, heatmap

    def enc_spec(self, x0, x1, x2, x3):
        x0 = self.ConvBlock1(x0)
        x0 = self.HourGlass1(x0)

        x1 = self.ConvBlock1_1(x1)
        x1 = self.HourGlass1_1(x1)

        x2 = self.ConvBlock1_2(x2)
        x2 = self.HourGlass1_2(x2)

        x3 = self.ConvBlock1_3(x3)
        x3 = self.HourGlass1_3(x3)

        return x0, x1, x2, x3

    def dec_spec(self, x0, x1, x2, x3):
        x0 = self.HourGlass4(x0)
        x0 = self.ConvBlock2(x0)

        x1 = self.HourGlass4_1(x1)
        x1 = self.ConvBlock2_1(x1)

        x2 = self.HourGlass4_2(x2)
        x2 = self.ConvBlock2_2(x2)

        x3 = self.HourGlass4_3(x3)
        x3 = self.ConvBlock2_3(x3)

        return x0, x1, x2, x3

    def forward_train(self, x0, x1, x2, x3):
        x0, x1, x2, x3 = self.enc_spec(x0, x1, x2, x3)
        ...
        x = self.DownBlock1(x)
        x = self.DownBlock2(x)  # 4, 128, 64, 64

        x = self.EncodeBlock1(x)  # 4, 128, 64, 64
        content_features1 = F.adaptive_avg_pool2d(x, 1).view(x.shape[0], -1)  # 4, 128
        x = self.EncodeBlock2(x)  # 4, 128, 64, 64
        content_features2 = F.adaptive_avg_pool2d(x, 1).view(x.shape[0], -1)  # 4, 128
        x = self.EncodeBlock3(x)  # 4, 128, 64, 64
        content_features3 = F.adaptive_avg_pool2d(x, 1).view(x.shape[0], -1)  # 4, 128
        x = self.EncodeBlock4(x)  # 4, 128, 64, 64
        content_features4 = F.adaptive_avg_pool2d(x, 1).view(x.shape[0], -1)  # 4, 128

        gap = F.adaptive_avg_pool2d(x, 1)   # 4, 128, 1, 1
        gap_logit = self.gap_fc(gap.view(x.shape[0], -1))  # 4, 4
        gap_weight = list(self.gap_fc.parameters())[0]  # 4, 128
        # gap_logit = self.gap_fc_new(gap.view(x.shape[0], -1))   # 4, 4
        # gap_weight = list(self.gap_fc_new.parameters())[0] # 4, 128
        gap = x * gap_weight.unsqueeze(2).unsqueeze(3)  # 4, 128, 64, 64

        gmp = F.adaptive_max_pool2d(x, 1)
        gmp_logit = self.gmp_fc(gmp.view(x.shape[0], -1))
        gmp_weight = list(self.gmp_fc.parameters())[0]
        # gmp_logit = self.gmp_fc_new(gmp.view(x.shape[0], -1))
        # gmp_weight = list(self.gmp_fc_new.parameters())[0]
        gmp = x * gmp_weight.unsqueeze(2).unsqueeze(3)

        cam_logit = torch.cat([gap_logit.unsqueeze(1), gmp_logit.unsqueeze(1)], 1)
        x = torch.cat([gap, gmp], 1)
        x = self.relu(self.conv1x1(x))  # 4, 128, 64, 64

        heatmap = torch.sum(x, dim=1, keepdim=True)

        if self.light:
            x_ = F.adaptive_avg_pool2d(x, 1)
            style_features = self.FC(x_.view(x_.shape[0], -1))  # 4, 128
        else:
            style_features = self.FC(x.view(x.shape[0], -1))
        ...
        x = self.DecodeBlock2(x, content_features3, style_features)
        x = self.DecodeBlock3(x, content_features2, style_features)
        x = self.DecodeBlock4(x, content_features1, style_features)  # 4, 128, 64, 64
        ...
        x0, x1, x2, x3 = x[0].unsqueeze(0), x[1].unsqueeze(0), x[2].unsqueeze(0), x[3].unsqueeze(0)
        out0, out1, out2, out3 = self.dec_spec(x0, x1, x2, x3)
        return out0, out1, out2, out3, cam_logit


    def get_params(self):
        specific_layers_enc = [self.ConvBlock1, self.HourGlass1, self.ConvBlock1_1, self.HourGlass1_1, \
                               self.ConvBlock1_2, self.HourGlass1_2, self.ConvBlock1_3, self.HourGlass1_3]
        specific_layers_dec = [self.HourGlass4, self.ConvBlock2, self.HourGlass4_1, self.ConvBlock2_1, \
                               self.HourGlass4_2, self.ConvBlock2_2, self.HourGlass4_3, self.ConvBlock2_3]
        specific_params_enc = [x.parameters() for x in specific_layers_enc]
        specific_params_dec = [x.parameters() for x in specific_layers_dec]
        specific_params = specific_params_enc + specific_params_dec
        specific_params = list(itertools.chain(*specific_params))

        cl = list(map(id, specific_params))
        shared_params = list(filter(lambda x: id(x) not in cl, self.parameters()))

        return specific_params, shared_params

    def copy_params(self):
        for target_param, param in zip(self.ConvBlock1_1.parameters(), self.ConvBlock1.parameters()):
            target_param.data.copy_(param.data)

        for target_param, param in zip(self.HourGlass1_1.parameters(), self.HourGlass1.parameters()):
            target_param.data.copy_(param.data)

        for target_param, param in zip(self.HourGlass4_1.parameters(), self.HourGlass4.parameters()):
            target_param.data.copy_(param.data)

        for target_param, param in zip(self.ConvBlock2_1.parameters(), self.ConvBlock2.parameters()):
            target_param.data.copy_(param.data)

        for target_param, param in zip(self.ConvBlock1_2.parameters(), self.ConvBlock1.parameters()):
            target_param.data.copy_(param.data)

        for target_param, param in zip(self.HourGlass1_2.parameters(), self.HourGlass1.parameters()):
            target_param.data.copy_(param.data)

        for target_param, param in zip(self.HourGlass4_2.parameters(), self.HourGlass4.parameters()):
            target_param.data.copy_(param.data)

        for target_param, param in zip(self.ConvBlock2_2.parameters(), self.ConvBlock2.parameters()):
            target_param.data.copy_(param.data)

        for target_param, param in zip(self.ConvBlock1_3.parameters(), self.ConvBlock1.parameters()):
            target_param.data.copy_(param.data)

        for target_param, param in zip(self.HourGlass1_3.parameters(), self.HourGlass1.parameters()):
            target_param.data.copy_(param.data)

        for target_param, param in zip(self.HourGlass4_3.parameters(), self.HourGlass4.parameters()):
            target_param.data.copy_(param.data)

        for target_param, param in zip(self.ConvBlock2_3.parameters(), self.ConvBlock2.parameters()):
            target_param.data.copy_(param.data)
...
class ConditionalSoftAdaLIN(nn.Module):
    def __init__(self, num_features, eps=1e-5):
        super(ConditionalSoftAdaLIN, self).__init__()
        self.norm = adaLIN(num_features, eps)
...
class Discriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=5):
        ...
        self.conv_cls = nn.utils.spectral_norm(
            nn.Conv2d(ndf * mult, 4, kernel_size=1, stride=1, padding=0, bias=False))
    ...
    def forward(self, input):
        ...
        gap_cls = torch.nn.functional.adaptive_avg_pool2d(x, 1)
        cls_logit = self.conv_cls(gap_cls)

        return out, cls_logit.view(cls_logit.size(0), cls_logit.size(1)), cam_logit, heatmap
